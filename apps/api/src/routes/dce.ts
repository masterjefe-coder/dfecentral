...
export async function dceRoutes(app: FastifyInstance) {
  app.get<{ Params: { chave: string } }>('/:chave', async (request, reply) => {
    ...
    const resultado = await db
      .select()
      .from(documentos)
      .where(and(eq(documentos.chaveAcesso, chave), eq(documentos.tipo, 'dce')))
      .limit(1);

    if (resultado.length === 0) {
      return reply.status(404).send({
        sucesso: false,
        erro: 'DC-e não encontrada',
      });
    }
  ...
  app.get('/', async (request, reply) => {
    ...
    const resultados = await db
      .select()
      .from(documentos)
      .where(and(eq(documentos.tipo, 'dce'), eq(documentos.cnpjEmitente, cnpj)))
      .limit(limite)
      .offset(offset);
