...
function normalizarRedirect(valor: string | undefined): string {
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/esqueci-senha', async (request) => {
    ...
    const url = `${baseWebUrl()}/auth/redefinir?token=${encodeURIComponent(token)}`;
  ...
  app.get('/google/iniciar', async (request, reply) => {
    ...
    const redirect = normalizarRedirect((request.query as Record<string, string | undefined>).redirect);
    if (!clientId || !clientSecret) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_indisponivel`);
    }

    const state = Buffer.from(JSON.stringify({ redirect }), 'utf8').toString('base64url');
    const callbackUrl = `${baseApiUrl()}/api/v1/auth/google/callback`;
  ...
  app.get('/google/callback', async (request, reply) => {
    ...
    if (!code || !state) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_invalido`);
    }

    const clientId = process.env.GOOGLE_CLIENT_ID;
    const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
    const redirectUri = `${baseApiUrl()}/api/v1/auth/google/callback`;
    if (!clientId || !clientSecret) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_indisponivel`);
    }
    ...
    if (!tokenResponse.ok) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_token`);
    }
    ...
    if (!profileResponse.ok) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_profile`);
    }

    const claims = (await profileResponse.json()) as { email?: string; name?: string };
    if (!claims.email) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_email`);
    }
    ...
    if (!usuario.apiKey) {
      return reply.redirect(`${baseWebUrl()}/auth/entrar?erro=google_key`);
    }
