...
- No OCI Email Delivery, crie o approved sender `contato@dfecentral.com.br` e gere as credenciais SMTP.
- Veja `DNS.md` para os valores sugeridos.

## Como subir
1. Copie `mailserver.env.example` para `mailserver.env`.
2. Ajuste `DEFAULT_RELAY_HOST`, `RELAY_USER` e `RELAY_PASSWORD` com a configuracao do OCI Email Delivery.
